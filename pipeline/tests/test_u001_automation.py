...
class U001AutomationPolicyTest(TestCase):
    ...
    def test_select_next_action_ignores_rd001_backlog_after_rd001_is_removed_from_controller(self):
        now = timezone.now()
        recent_coin = MigratedCoin.objects.create(
            mint_address='AUTO_RD001_RECENT_ONLY',
            anchor_event=now - timedelta(hours=6),
        )
        historical_coin = MigratedCoin.objects.create(
            mint_address='AUTO_RD001_HIST_ONLY',
            anchor_event=now - timedelta(days=5),
        )
        for coin, pool in (
            (recent_coin, 'POOL-RD001-RECENT-ONLY'),
            (historical_coin, 'POOL-RD001-HIST-ONLY'),
        ):
            PoolMapping.objects.create(
                coin=coin,
                pool_address=pool,
        ...
        RawTransaction.objects.create(
            ...
            tx_signature='sig-auto-rd001-only',
            trade_type='BUY',
            wallet_address='wallet-auto-rd001-only',
            token_amount=10,
            sol_amount=1,
            pool_address='POOL-RD001-RECENT-ONLY',
            tx_fee='0.001',
            lp_fee=0,
            protocol_fee=0,
            coin_creator_fee=0,
    ...
    def test_select_next_action_enters_failure_pause_after_repeated_errors(self):
        ...
        state = get_or_create_state()
        state.last_action = 'pool_mapping_recent'
        state.last_action_status = 'error'
...
class AutomateU001CommandTest(TestCase):
    ...
    def test_command_rejects_removed_rd001_force_action(self):
        with self.assertRaisesMessage(
            CommandError,
            "Unsupported force_action 'rd001_recent' for automate_u001",
        ):
            call_command(
                'automate_u001',
                force_action='rd001_recent',
                skip_snapshot=True,
                skip_repair=True,
            )
    ...
    def test_command_updates_state_and_runs_selected_action(self):
        ...
        for index in range(4):
            coin = MigratedCoin.objects.create(
                mint_address=f'AUTO_CMD_RECENT_{index}',
                anchor_event=now - timedelta(hours=index + 1),
            )
            if index == 0:
                PoolMapping.objects.create(
        ...
        self.assertEqual(
            inner_call.call_args_list[1].kwargs['steps'],
            'pool_mapping',
        )
