...
**What you can DO after this phase:** Call `get_panel_slice(["mint_A", "mint_B"], ["FL-001", "FL-002"], simulation_time=datetime(...))` and receive a merged panel where each row has OHLCV data and holder data for the same coin at the same timestamp, with PIT enforced (no future data leaking in). Call `get_universe_members(simulation_time)` and see only coins that had graduated by that time.
...
| 4.A.3 | Design alignment logic. For U-001, both FL-001 and FL-002 share 5-min resolution, so forward-fill is not needed yet. But the alignment function must: (a) match rows from FL-001 and FL-002 on coin + timestamp (inner join — JK-001), (b) handle the case where FL-001 has a row at timestamp T but FL-002 doesn't (or vice versa) — inner join drops that row, (c) produce wide-format output with columns from both layers. When layers with different resolutions are added later, forward-fill activates: carry last known value from slower layer, attach `{layer_id}_{short_name}_staleness_minutes` field (0 for actual, increasing for filled). | u001_data_specification.md JK-001: inner join, row-level existence, forward-fill with staleness field naming convention `{layer_id}_{short_name}_staleness_minutes`. u001_dataset_implementation_record.md WDP10: application code (Python). |
...
| 4.B.3 | Write tests for `get_universe_members`. Setup: create 3 MigratedCoins with different anchor_events (e.g., 10:00, 11:00, 12:00). Assert: at simulation_time=10:30, only coin 1 is returned. At 11:30, coins 1 and 2. At 12:30, all three. | The core PIT correctness test for universes |
| 4.B.4 | Write tests for error handling. Call `get_reference_data` with a coin not in the universe — assert raises ValueError. Call with valid coin but no data — assert returns empty. | WDP13 error handling |
...
| 4.D.2 | Write alignment test. Setup: create coin with both FL-001 and FL-002 data. FL-001 has candles at 10:00, 10:05, 10:10. FL-002 has snapshots at 10:00, 10:05 (but NOT 10:10). Test: `get_panel_slice` requesting both layers returns only 2 rows (10:00 and 10:05) — inner join drops 10:10 because FL-002 is missing. Each row has columns from both layers. | JK-001 inner join |
| 4.D.3 | Write empty-result test. Create a coin that graduated but has no candle data (coin died immediately). `get_panel_slice` returns empty, not an error. | WDP13: valid request, no data → empty |
| 4.D.4 | Write scope validation test. Call `get_panel_slice` with a coin that doesn't exist in MigratedCoin. Assert raises ValueError. | WDP13: invalid request → error |
...
- [ ] `get_panel_slice` with nonexistent coin raises ValueError
- [ ] `get_panel_slice` with valid coin but no data returns empty (not an error)
- [ ] `get_reference_data` with nonexistent coin raises ValueError
- [ ] `get_reference_data` with valid coin but no RawTransaction data returns empty
...
| Alignment in Python (WDP10) is slow for large datasets | Performance bottleneck at scale | Acceptable for U-001 scale (~1000 rows per coin). Profile if performance becomes an issue. WDP10 chose Python over SQL for flexibility. |
